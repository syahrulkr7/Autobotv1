...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (!username || !message || isNaN(count) || count <= 0 || !Number.isInteger(count)) {
    ...
    try {
      const response = await axios.get(`https://eurixapi.onrender.com/ngl/spam?username=&message=&count=${encodeURIComponent(username)}&message=${encodeURIComponent(message)}&count=${encodeURIComponent(count)}`); 

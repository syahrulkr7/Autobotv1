...
module.exports.run = async ({ api, event, args }) => {
	...
	if (args[0] === "gen") {
		try {
			const response = await axios.get("https://eurixapi.onrender.com/tempmail/gen");
			const responseData = response.data.email;
			api.sendMessage(`➪${responseData}`, event.threadID);
		} catch (error) {
		...
		const email = args[1];
		try {
			const response = await axios.get(`https://eurixapi.onrender.com/tempmail/message?email=${email}`);
